...
const StudentCard = ({ student }) => {
    const { t } = useTranslation('pages/participantAdminPage'); // Initialize translation
    ...
    return (
        <motion.div
            className="shadow-lg rounded-lg bg-white p-6 mb-6 transform transition-transform duration-300 hover:scale-105 overflow-hidden"
                ...
                <div className="mb-2 text-gray-600">
                    ...
                    {student.gender && (
                        <div className="flex items-center mb-2">
                            ...
                            <p className="truncate">{student.gender}</p>
                        </div>
                    )}
                    {student.cin && (
                        <div className="flex items-center mb-2">
                    ...
                    {student.age && (
                        <div className="flex items-center mb-2">
                            <p className="truncate">{student.age} {t('labels.yearsOld')}</p>
                        </div>
                    )}
                    {isExpanded && (
                        <>
...
const ParticipantsAdminPage = () => {
    ...
    useEffect(() => {
        const fetchStudents = async () => {
            try {
                const response = await axiosInstance.get(`/entreprises/${entrepriseId}/participants`);
                setStudents(response.data);
                setFilteredStudents(response.data); // Show all students initially
            } catch (error) {
                console.error("Error fetching students:", error);
                toast.error(t('messages.loadError')); // Use translation for error message
            }
        };

        const fetchEnterpriseData = async () => {
            try {
                const response = await axiosInstance.get(`/hierarchical-units/entreprise/${entrepriseId}`);
    ...
    const getAllDescendants = (unitId, allUnits) => {
        ...
        children.forEach(child => {
            allDescendants = allDescendants.concat(getAllDescendants(child.value, allUnits));
        });
    ...
    const filterStudents = (filterChain) => {
        ...
        if (selectedUnitIds.length > 0) {
            ...
            selectedUnitIds.forEach(unitId => {
                const allDescendants = getAllDescendants(unitId, options);
    ...
    return (
        <div className="max-w-7xl mx-auto p-6 bg-gray-100">
            <h1 className="text-3xl font-bold mb-6 text-center">{t('titles.participants')}</h1>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                {filteredStudents.length > 0 ? (
                    ...
                    <p className="text-center text-gray-500 col-span-3">{t('messages.noStudents')}</p>
                )}
            </div>
        </div>
    );
